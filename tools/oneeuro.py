"""1€ (One Euro) filter — adaptive low-pass for the raw MediaPipe landmark stream.

Copied verbatim from D:\\asl-synthesis\\scripts\\oneeuro.py so E:\\ASL_Game stays
self-contained (the avatar repo lives on another drive and isn't always present).
Apply this to landmarks BEFORE saving training data: it smooths hard when the hand is
slow (kills jitter) and loosens when the hand is fast (preserves the sharp peaks of a
sign's contact/peak frames). Reference: Casiez, Roussel, Vogel, "1€ Filter" (CHI 2012).
"""
...
class OneEuroVector:
    ...
    occlusion gaps must survive downstream so they can be flagged, not smoothed over.
    """
