...
def _draw_scorecard(img, result) -> None:
    x, y, line = 12, 34, 30
    ...
    for p in result.params:
        col = (0, 200, 0) if p.cleared else (0, 0, 255)
        ...
        cv2.putText(img, f"{p.name:<22}{p.score:0.2f} / {p.threshold:0.2f} [{tag}]",
                    (x + 140, y), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 255, 255), 1, cv2.LINE_AA)
        y += line
...
def main(sign_name: str = "COFFEE", camera_index: int = 0) -> None:
    ...
    with Capture() as capture:
        while True:
            ...
            _draw_scorecard(bgr, verify(buffer, sign))
            cv2.imshow(f"ASL_Game Phase 3 - verify {sign_name} (q to quit)", bgr)
